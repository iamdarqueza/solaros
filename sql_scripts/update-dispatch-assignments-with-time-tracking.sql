...
ADD COLUMN planned_end_time TIMESTAMP WITH TIME ZONE,
...
ADD COLUMN on_time BOOLEAN DEFAULT NULL,
ADD COLUMN duration_expected_minutes INTEGER,
ADD COLUMN duration_actual_minutes INTEGER;

-- Add indexes for the new time-related columns for better query performance
CREATE INDEX idx_dispatch_assignments_planned_start_time ON dispatch_assignments(planned_start_time);
CREATE INDEX idx_dispatch_assignments_planned_end_time ON dispatch_assignments(planned_end_time);
CREATE INDEX idx_dispatch_assignments_actual_start_time ON dispatch_assignments(actual_start_time);
CREATE INDEX idx_dispatch_assignments_actual_end_time ON dispatch_assignments(actual_end_time);
CREATE INDEX idx_dispatch_assignments_on_time ON dispatch_assignments(on_time);
CREATE INDEX idx_dispatch_assignments_delay_minutes ON dispatch_assignments(delay_minutes);
