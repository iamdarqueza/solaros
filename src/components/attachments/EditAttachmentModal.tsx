...
import { CreateAttachmentData, AttachmentWithDetails, attachmentsService } from '@/services/attachmentsService';
...
interface EditAttachmentModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (data: Partial<CreateAttachmentData>) => Promise<void>;
...
export default function EditAttachmentModal({
  ...
  const [formData, setFormData] = useState<Partial<CreateAttachmentData>>({});
  ...
  useEffect(() => {
    if (attachment) {
      setFormData({
        ...
        assigned_vehicle_id: attachment.assigned_vehicle_id || '',
        status: attachment.status,
        last_known_location: attachment.last_known_location,
        beacon_id: attachment.beacon_id || '',
        maintenance_interval_hrs: attachment.maintenance_interval_hrs,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const submitData: Partial<CreateAttachmentData> = {
        ...formData,
      ...
      <div className="p-6 max-h-[70vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            ...
            />
            <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
              Click on the map to update the equipment's location
            </p>
          </div>
