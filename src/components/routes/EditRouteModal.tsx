...
import { routesService, RouteData, UpdateRouteData, Location } from '@/services/routesService';
import { formatDuration } from '@/utils/timeUtils';

interface EditRouteModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (data: UpdateRouteData) => Promise<void>;
  isLoading: boolean;
...
export default function EditRouteModal({ isOpen, onClose, onSubmit, isLoading, route }: EditRouteModalProps) {
  ...
  useEffect(() => {
    if (route && isOpen) {
      ...
      // Set origin location if available
      if (route.origin_lat && route.origin_lng) {
        const originData = {
          coordinates: [route.origin_lng, route.origin_lat] as [number, number],
          placeName: route.origin_location_name || `${route.origin_lat.toFixed(4)}, ${route.origin_lng.toFixed(4)}`
        };
        setOriginLocation(originData);
        setOriginSearch(originData.placeName);
      }
      
      // Set destination location if available
      if (route.destination_lat && route.destination_lng) {
        const destinationData = {
          coordinates: [route.destination_lng, route.destination_lat] as [number, number],
          placeName: route.destination_location_name || `${route.destination_lat.toFixed(4)}, ${route.destination_lng.toFixed(4)}`
        };
        setDestinationLocation(destinationData);
        setDestinationSearch(destinationData.placeName);
      }
      
      // Set existing distance and duration
      setEstimatedDistance(route.estimated_distance_km || null);
      setEstimatedDuration(route.estimated_duration_minutes || null);
    }
  }, [route, isOpen]);
  ...
  useEffect(() => {
    ...
    import('mapbox-gl').then((mapboxgl) => {
      ...
      // Add existing markers after map loads
      map.on('load', () => {
        if (originLocation) {
          addOrUpdateMarker('origin', originLocation.coordinates[0], originLocation.coordinates[1], originLocation.placeName);
        }
        if (destinationLocation) {
          addOrUpdateMarker('destination', destinationLocation.coordinates[0], destinationLocation.coordinates[1], destinationLocation.placeName);
        }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const routeData: UpdateRouteData = {
      id: route.id,
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      if (originSearch && (!originLocation || originSearch !== originLocation.placeName)) {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      if (destinationSearch && (!destinationLocation || destinationSearch !== destinationLocation.placeName)) {
