...
import { CreateVehicleData, VehicleWithDetails, vehiclesService } from '@/services/vehiclesService';
...
interface EditVehicleModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (vehicleData: Partial<CreateVehicleData>) => Promise<void>;
...
const EditVehicleModal: React.FC<EditVehicleModalProps> = ({
  ...
  useEffect(() => {
    if (vehicle) {
      setFormData({
        ...
        vin: vehicle.vin || '',
        av_enabled: vehicle.av_enabled || false,
        is_electric: vehicle.is_electric || false,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Failed to update vehicle. Please try again.';
        ...
        <form onSubmit={handleSubmit} className="flex flex-col flex-1 min-h-0">
          ...
          <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-4">
            ...
            <div className="space-y-1">
              <p className="text-sm text-gray-600 dark:text-gray-400">
                <span className="font-medium">Plate Number:</span> {vehicle.plate_number}
              </p>
              <p className="text-sm text-gray-600 dark:text-gray-400">
                <span className="font-medium">Type:</span> {
                  vehicle.vehicle_type === 'logistics_freight' ? 'Logistics & Freight' :
                  vehicle.vehicle_type === 'electric_autonomous' ? 'Electric & Autonomous' :
                  vehicle.vehicle_type === 'utility_support' ? 'Utility & Support' :
                  vehicle.vehicle_type === 'simulated_truck' ? 'Simulated Truck' :
          ...
          <div>
            ...
            <p className="mt-2 text-xs text-gray-500 dark:text-gray-400">
              Click on the map to update the vehicle's location, or use "Clear Location" to remove it
            </p>
          </div>
          </div>
